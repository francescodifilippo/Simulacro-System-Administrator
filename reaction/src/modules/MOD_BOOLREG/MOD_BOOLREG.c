...
#include "../../intern.h"
#include "../../blib.h"
/* function for check boolean regular expresion */
int boolean_reg_exp(char *, char *);


BVariant* function_MOD_BOOLREG( BSContext *context, BList *args )
{
	int narg;
	BVariant *arg1, *arg2;
	...
	if( narg == 2 )
	{
		...
		bvariant_free( arg2 );
        	...
        	/* check boolean regular expresion */
		status=boolean_reg_exp(string,regexp);
	}	
...
int boolean_reg_exp(char *string, char *pattern)
{
        int status;
        regex_t re;
