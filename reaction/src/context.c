...
void bs_context_exec_print( BSContext* cx , int socket)
{
  BVariant *v;
  ...
  if( v->type == BV_DOUBLE )
  {
    ...
    writebuffer=(char *) malloc((sizeof (char)) * 2048);
    snprintf(writebuffer,2048,"%g", number);
    write(socket, writebuffer, strlen(writebuffer));
  }
  ...
  if( v->type == BV_STRING )
  {
    ...
    writebuffer=(char *) malloc((sizeof (char)) * strlen(string)+1);
    snprintf(writebuffer,strlen(string)+1,"%s", string);
    write(socket, writebuffer, strlen(writebuffer));
  }
...
void bs_context_execute( BSContext *context, int socket )
{
  ...
  while( context->program_counter >= 0 )
  {
    ...
    {
      bs_context_exec_print( context, socket);
      context->program_counter++;
    }
...
bs_eval_builtin_function( BSContext *context, char *function, int narg )
{
  ...
  char sofilename[MAXLEN]=BASEDIRNAME;
  char sofunction[MAXLEN]=BASEFUNCNAME;
  BVariant *(*sharedobject)(BSContext*,BList*)=NULL;
  void *sohandle=NULL;
  const char *error;
  int isshared=0;
  ...
  if ( !func ) 
  {
	if (strstr (function, "MOD_"))
	{
		/* controlliamo se esiste la libreria e la funzione in questione */
		strncat (sofilename, function, MAXLEN);
        	strncat (sofilename, ".so", MAXLEN); 
        	strncat (sofunction, function, MAXLEN);
		if (lstat(sofilename, &lstatbuf) != -1)
        	{
       	        	if(S_ISREG(lstatbuf.st_mode))
               		{
				sohandle = dlopen (sofilename, RTLD_LAZY);
        			if (!sohandle)
        			{
        			...
        			{
					sharedobject=dlsym(sohandle, sofunction);
  ...
  if (isshared)
  {
	result = (*sharedobject)( context, args );
	dlclose(sohandle);
  }
  else
  {
