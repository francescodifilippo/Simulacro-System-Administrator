...
void
bs_module_set_code( BSModule *module, char* code, int socket )
{
  if( !module ) return;
  ...
  module->bytecodes = reaction_parse( module->code, socket ); /* in gram.y */
}

void
bs_module_load_file( BSModule *module, char *filename, int socket )
{
  FILE *f;
  int filesize;
  char *buffer;
  ...
  bs_module_set_code( module, buffer, socket );

  b_free( buffer );
}

void
bs_module_execute ( BSModule *module, int socket )
{
  BSContext *context;
  ...
  bs_context_execute( context, socket );
...
void
bs_interpreter_run( BSInterpreter *interpreter, char *code, int socket )
{
  BSModule *module;
  module = bs_module_create( "reaction" );
  bs_module_set_code( module, code, socket );
  bs_module_execute( module, socket );
  bs_module_destroy( module );
}

void
bs_interpreter_run_file( BSInterpreter *interpreter, char *filename, int socket )
{
  BSModule *module;
  module = bs_module_create( "reaction" );
  bs_module_load_file( module, filename, socket );
  bs_module_execute( module, socket );
  bs_module_destroy( module );
}
